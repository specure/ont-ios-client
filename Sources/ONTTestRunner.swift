...
///
let ONTTestRunnerProgressUpdateInterval: TimeInterval = 0.1 // seconds
...
///
public enum ONTTestRunnerPhase: Int {
    case none = 0
...
///
public protocol ONTTestRunnerDelegate {
    
    ///
    func testRunnerDidStartPhase(_ phase: ONTTestRunnerPhase)
    
    ///
    func testRunnerDidFinishPhase(_ phase: ONTTestRunnerPhase)
    
    ///
    func testRunnerDidUpdateProgress(_ progress: Float, inPhase phase: ONTTestRunnerPhase)
    
    ///
    func testRunnerDidMeasureThroughputs(_ throughputs: NSArray, inPhase phase: ONTTestRunnerPhase)
...
///
open class ONTTestRunner: NSObject, ONTTestWorkerDelegate, RMBTConnectivityTrackerDelegate {
    ...
    ///
    fileprivate var workers = [ONTTestWorker]()
    
    ///
    fileprivate let delegate: ONTTestRunnerDelegate
    
    ///
    fileprivate var phase: ONTTestRunnerPhase = .none
    ...
    ///
    public init(delegate: ONTTestRunnerDelegate) {
    ...
    fileprivate func continueWithTestParams(_ testParams: SpeedMeasurementResponse/*RMBTTestParams*/) {
        ...
        for i in 0..<testParams.numThreads {
            let worker = ONTTestWorker(delegate: self, delegateQueue: workerQueue, index: UInt(i), testParams: testParams)
            ...
            NotificationCenter.default.addObserver(
                self,
                selector: #selector(ONTTestRunner.applicationDidSwitchToBackground(_:)),
    ...
    ///
    open func testWorker(_ worker: ONTTestWorker, didFinishDownlinkPretestWithChunkCount chunks: UInt, withTime duration: UInt64) {
        //ASSERT_ON_WORKER_QUEUE();
    ...
    ///
    open func testWorkerDidStop(_ worker: ONTTestWorker) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .Init, "Invalid state")
        assert(!dead, "Invalid state")
    ...
    ///
    open func testWorker(_ worker: ONTTestWorker, didMeasureLatencyWithServerNanos serverNanos: UInt64, clientNanos: UInt64) {
        //ASSERT_ON_WORKER_QUEUE();
    ...
    ///
    open func testWorkerDidFinishLatencyTest(_ worker: ONTTestWorker) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .latency, "Invalid state")
        assert(!dead, "Invalid state")
    ...
    ///
    open func testWorker(_ worker: ONTTestWorker, didStartDownlinkTestAtNanos nanos: UInt64) -> UInt64 {
        //ASSERT_ON_WORKER_QUEUE();
    ...
    ///
    open func testWorker(_ worker: ONTTestWorker, didDownloadLength length: UInt64, atNanos nanos: UInt64) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .down, "Invalid state")
        assert(!dead, "Invalid state")
    ...
    ///
    open func testWorkerDidFinishDownlinkTest(_ worker: ONTTestWorker) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .down, "Invalid state")
        assert(!dead, "Invalid state")
        ...
        if markWorkerAsFinished() {
            ...
            startPhase(.initUp, withAllWorkers: true, performingSelector: #selector(ONTTestWorker.startUplinkPretest), expectedDuration: testParams.pretestDuration, completion: nil)
        }
    }
    
    ///
    open func testWorker(_ worker: ONTTestWorker, didFinishUplinkPretestWithChunkCount chunks: UInt) {
        //ASSERT_ON_WORKER_QUEUE();
        ...
        if markWorkerAsFinished() {
            ...
            startPhase(.up, withAllWorkers: true, performingSelector: #selector(ONTTestWorker.startUplinkTest), expectedDuration: testParams.duration, completion: nil)
        }
    }
    
    ///
    open func testWorker(_ worker: ONTTestWorker, didStartUplinkTestAtNanos nanos: UInt64) -> UInt64 {
        //ASSERT_ON_WORKER_QUEUE();
    ...
    ///
    open func testWorker(_ worker: ONTTestWorker, didUploadLength length: UInt64, atNanos nanos: UInt64) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .up, "Invalid state")
        assert(!dead, "Invalid state")
    ...
    ///
    open func testWorkerDidFinishUplinkTest(_ worker: ONTTestWorker) {
        //ASSERT_ON_WORKER_QUEUE();
        assert(phase == .up, "Invalid state")
        assert(!dead, "Invalid state")
    ...
    ///
    open func testWorkerDidFail(_ worker: ONTTestWorker) {
        //ASSERT_ON_WORKER_QUEUE();
    ...
    ///
    fileprivate func setPhase(_ phase: ONTTestRunnerPhase) {
        if self.phase != .none {
    ...
    ///
    fileprivate func startPhase(_ phase: ONTTestRunnerPhase, withAllWorkers allWorkers: Bool, performingSelector selector: Selector!,
