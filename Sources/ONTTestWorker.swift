...
///
public enum ONTTestWorkerState: Int {
    case initialized
...
public protocol ONTTestWorkerDelegate {
    
    ///
    func testWorker(_ worker: ONTTestWorker, didFinishDownlinkPretestWithChunkCount chunks: UInt, withTime duration: UInt64)
    
    ///
    func testWorker(_ worker: ONTTestWorker, didMeasureLatencyWithServerNanos serverNanos: UInt64, clientNanos: UInt64)
    
    ///
    func testWorkerDidFinishLatencyTest(_ worker: ONTTestWorker)
    
    ///
    func testWorker(_ worker: ONTTestWorker, didStartDownlinkTestAtNanos nanos: UInt64) -> UInt64
    
    ///
    func testWorker(_ worker: ONTTestWorker, didDownloadLength length: UInt64, atNanos nanos: UInt64)
    
    ///
    func testWorkerDidFinishDownlinkTest(_ worker: ONTTestWorker)
    
    ///
    func testWorker(_ worker: ONTTestWorker, didFinishUplinkPretestWithChunkCount chunks: UInt)
    
    ///
    func testWorker(_ worker: ONTTestWorker, didStartUplinkTestAtNanos nanos: UInt64) -> UInt64
    
    ///
    func testWorker(_ worker: ONTTestWorker, didUploadLength length: UInt64, atNanos nanos: UInt64)
    
    ///
    func testWorkerDidFinishUplinkTest(_ worker: ONTTestWorker)
    
    ///
    func testWorkerDidStop(_ worker: ONTTestWorker)
    
    ///
    func testWorkerDidFail(_ worker: ONTTestWorker)
}

///
open class ONTTestWorker: NSObject, GCDAsyncSocketDelegate {
    
    // Test parameters
    fileprivate var params: SpeedMeasurementResponse
    
    /// Weak reference to the delegate
    fileprivate let delegate: ONTTestWorkerDelegate
    
    /// Current state of the worker
    fileprivate var state: ONTTestWorkerState = .initialized
    ...
    ///
    public init(delegate: ONTTestWorkerDelegate, delegateQueue: DispatchQueue, index: UInt, testParams: SpeedMeasurementResponse) {
