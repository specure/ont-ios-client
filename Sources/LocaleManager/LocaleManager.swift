...
public class LocaleManager: NSObject {
    ...
    */
    ...
    public class var isUseSystemLanguage: Bool {
        get {
            if let value = UserDefaults.standard.object(forKey: "LocaleManagerUseSystemLanguage") as? Bool {
                return value
            }
            else {
                return false
        ...
        set {
            UserDefaults.standard.set(newValue, forKey: "LocaleManagerUseSystemLanguage")
    ...
    */
    ...
    @objc public class func apply(locale: Locale?, animated: Bool = true) {
        ...
        if let locale = locale {
            let identifier = locale.languageCode ?? locale.identifier
            setLocale(identifiers: [identifier])
            semantic = locale.isRTL ? .forceRightToLeft : .forceLeftToRight
            isUseSystemLanguage = false
        } else {
            removeLocale()
            semantic = Locale.baseLocale.isRTL ? .forceRightToLeft : .forceLeftToRight
            isUseSystemLanguage = true
        }
